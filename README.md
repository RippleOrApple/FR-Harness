...
再次提醒：`--env-file .env` 会把明文凭据注入容器进程环境。不要把 `.env` 烘焙进镜像或提交到仓库；有条件时使用容器平台的 secret 挂载能力。
...
demo/
  mock_repair_demo.py
tests/
temp/task-*/     # 每个实施 Task 的 GOAL 与过程记录
Dockerfile
.gitlab-ci.yml
SPEC.md
PLAN.md
